...
using static OscCore.OscActionPair;
...
namespace OscCore
{
    public sealed unsafe class OscServer : IDisposable
    {
        ...
        static readonly Dictionary<OSCAction, OscActionPair> k_SingleCallbackToPair = 
            new Dictionary<OSCAction, OscActionPair>();
        ...
        public bool TryAddMethod(string address, OSCAction valueReadMethod)
        {
        ...
        public bool RemoveMethod(string address, OSCAction valueReadMethod)
        {
        ...
        /// <param name="address">The OSC address to associate a method with</param>
        /// <returns>True if the address was valid & methods associated with it, false otherwise</returns>
        public bool TryAddMethodPair(string address, OSCAction read, Action mainThread) => 
        ...
        public void ParseBuffer(int byteLength)
        {
            ...
            if (*bufferLongPtr != Constant.BundlePrefixLong)
            {
                ...
                {
                    HandleCallbacks(methodPair, parser, bufferPtr, addressLength);
                }
            ...
            do
            {
                ...
                while (MessageOffset < byteLength && !recurse)
                {
                    ...
                    {
                        HandleCallbacks(bundleMethodPair, parser, contentPtr, bundleAddressLength);
                    }
        ...
        void HandleCallbacks(OscActionPair pair, OscParser messageValues, byte* bufferPtr, int addressLength)
        {
            ...
            var addr = string.Copy(stringBuffer);

            // call the value read method associated with this OSC address    
            pair.ValueRead(messageValues.MessageValues, addr);
        ...
        void TryMatchPatterns(OscParser parser, byte* bufferPtr, int addressLength)
        {
            ...
            if (AddressSpace.TryMatchPatternHandler(stringBuffer, m_PatternMatchedMethods))
            {
                ...
                {
                    matchedMethod.ValueRead(parser.MessageValues, bufferCopy);
